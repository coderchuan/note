...
* 配置文件:`/etc/ntp.conf`
    ...
    * `server IP OPTION`
        * 含义:指定ntp的上层服务器。当上层服务器的上层服务器无法与上层服务器通信时,使用此上层服务器的客户端亦无法正常连接。因此若当此计算机作为时间服务器通常会指定一项指向本地地址的配置`server localhost iburst`,以保证客户端始终正常使用
        * IP:上层ntp服务器的IP地址或域名
        * OPTION:上层服务器的参数
            * `key KEY_ID`:当上层服务器启用了`notrust`项时,此项必须被指定。其中`KEY_ID`表示需要被验证的`key_file`文件中的ID值
