...
* 方法(设obj为str实例,[unicode字体信息网](https://www.fileformat.info/info/unicode/category/index.htm))
    * 判断
        ...
        * `str.startswith(prefix[, start[, end]])`:如果字符串在索引start到end的子串以prefix开头则返回True
    * 查找 
        * `str.find(sub[, start[, end]])`:返回在字符串的索引start和end间找到的sub子串的最小起始索引
        * `str.rfind(sub[, start[, end]])`:返回在字符串的索引start和end间找到的sub子串的最大起始索引
        * `str.index(sub[, start[, end]])`:返回在字符串的索引start和end间找到的sub子串的最小起始索引,未查找到将会抛出`ValueError`
        * `str.rindex(sub[, start[, end]])`:返回在字符串的索引start和end间找到的sub子串的最大起始索引,未查找到将会抛出`ValueError`
        * `str.count(sub[, start[, end]])`:返回子串sub在字符索引start和end间的重复的次数
    * 大小写转换
        * `str.lower()`:返回字符串的小写形式  
        * `str.upper()`:返回字符串的大写形式 
        * `str.title()`:返回str中每个单词首字母大写且非首字母小写的字符串
        * `str.capitalize()`:返回首字母大写,其余字母小写的字符串 
        * `str.swapcase()`:返回大写字母转为小写且小写字母转为大写后的字符串
        * `str.casefold()`:返回消除歧义后的字母(类似于把所有大写转为小写,把不同形态的同一个字母转换为同一种字母,如会把'ß'转换为'ss')
    * 对齐
        * `str.center(width[, fillchar])`
            * 如果width大于等于字符串长度,返回长度为width的且原字符居中的字符串,原字符两侧由fillchar所指定的字符填充,默认空格
            * 如果width小于字符串长度,则返回原字符串  
        * `str.ljust(width[, fillchar])`
            * 如果width大于等于字符串长度,则返回宽度为width的左对齐字符串,填充字符为fillchar,默认为空格。 
            * 如果width小于字符串长度,则返回原字符串  
        * `str.rjust(width[, fillchar])`
            * 如果width大于等于字符串长度,则返回宽度为width的右对齐字符串,填充字符为fillchar,默认为空格。 
            * 如果width小于字符串长度,则返回原字符串  
        * `str.zfill(width)`
            * 如果width大于等于字符串长度,则返回宽度为width的右对齐字符串,填充字符为'0'。正负号在字符串起始位置
            * 如果width小于字符串长度,则返回原字符串  
    * 分割与拼接
        * `str.partition(sep)`:返回使用第一次出现的sep作为分隔符把str分隔成三部分的元组。第一部分为分隔符之前的字符串,第二部分为分隔符本身,第三部分为分隔符后的字符串。如果在str中未找到分隔符,则第一部分为空字符串,第二部分为分隔符本身,第三部分为空字符串
        * `str.rpartition(sep)`:返回使用最后一次出现的sep作为分隔符把str分隔成三部分的元组。第一部分为分隔符之前的字符串,第二部分为分隔符本身,第三部分为分隔符后的字符串。如果在str中未找到分隔符,则第一部分为空字符串,第二部分为分隔符本身,第三部分为空字符串
        * `str.split(sep=None, maxsplit=-1)`:返回使用sep作为分隔符对str进行从左至右切割后的元组,如果maxsplit>=0,则只切割maxsplit次。如果sep的值为None,则默认使用空白字符进行切割 
        * `str.rsplit(sep=None, maxsplit=-1)`:返回使用sep作为分隔符对str进行从右至左切割后的元组,如果maxsplit>=0,则只切割maxsplit次。如果sep的值为None,则默认使用空白字符进行切割 
        * `str.splitlines([keepends])`:把str拆分为行并返回,默认不保留分隔符,如果指定keepends=Ture且保留分隔符在行尾。分隔符如下:
            * \n 
            * \r 
            * \r\n 
            * \v
            ...
            * \u2028
            * \u2029
        * `str.join(iterable)`:返回使用str对可迭代对象iterable(元素必须为字符串或字符)进行拼接。字符串也属性可迭代对象
    * 首尾处理
        * `str.strip([chars])`:返回移除前后字符后的字符串,移除的字符列表由chars指定,若未指定,则移除空白字符  
        * `str.lstrip([chars])`:返回移除前导字符后的字符串,移除的字符列表由chars指定,若未指定,则移除空白字符 
        * `str.rstrip([chars])`:返回移除末尾字符后的字符串,移除的字符列表由chars指定,若未指定,则移除空白字符 
        * `str.removeprefix(prefix, /)`:返回去除前缀字符串prefix剩下的字符串。如果前缀不是prefix则返回原字符串的副本
        * `str.removesuffix(suffix, /)`:返回去除后缀字符串suffix剩下的字符串。如果前缀不是suffix则返回原字符串的副本
    * 格式化
        * `str.format(*args, **kwargs)`:返回格式化后的字符串,同`format()`格式化
        * `str.format_map(mapping)`:返回格式化后的字符串,同`format_map()`格式化
    * 映射表
        * `static str.maketrans(x[, y[, z]])`:生成字典转换表
            * 只传入x参数:x的类型必须为字典,且形式必须为字典:{单个字符:字符串}或{unicode码(整数):字符串}
                * 返回值:如果传入参数的键不是整数而是单个字符,则把那个字符转换为十进制的unicode整数,返回字典:{unicode码:字符串} 
            * 只传入x和y:x,y必须为字符串,且x和y的长度必须相等
                * 返回值:把x每个字符转换为unicode码并与y的每个字符对应,返回字典:{unicode码:字符}
            * 传入x,y,z:x,y,z必须为字符串,且x和y的长度必须相等。z没有要求
                * 返回值:把x每个字符转换为unicode码并与y的每个字符对应,z中的每个字符转换为unicode码并与None对应,返回字典:{unicode码:字符} 
        * `str.translate(table)`:把str中的字符通过table转换为特定的字符串。table为字典转换表,可由`static str.maketrans(x[, y[, z]])`生成。
    * 替换
